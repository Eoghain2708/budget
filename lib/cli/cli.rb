...
class CLI
  ...
  def run(argv)
    ...
    when "trading"
      ...
      when "positions", "pos"
        ...
        Budget::API::Trading212Formatter.format_positions(data)
      when "orders"
        data = client.orders
        pp data
      when "exports"
    ...
    when 'limit', 'l'
      action = argv&.shift
      errorise('You must include an action: add | delete | edit | see') unless action
  ...
  def get_date_for_edit_and_delete(argv, options)
    from = argv.shift
    from = if from
             PeriodDefiner.define_day(from) || Date.today
           else
             Date.today
           end

    to = if options.dig(:to)
           PeriodDefiner.define_day(to)
         else
           from
         end
